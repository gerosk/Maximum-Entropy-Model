...
excel = pd.read_excel (r'C:\Users\geros\OneDrive\Υπολογιστής\Διπλωματική\Data_michalis\All parameters\AllParams.xlsx')
...
'''Μοντελοποίση με χρήση του kg, για εύρεση kag, kbr'''


x_data = t[0:51]
y_data = m0
y_data1=C
def moments(t,y,params):
    m0 = y[0]
    m1 = y[1]
    m2 = y[2]
    m3 = y[3]
    ...
    def moment1_A(L,λ):
        return(kag*((L**3+λ**3)**(1/3)/2-L)*sol(L)*sol(λ))
    
    
    dm1dt_A, err1_A=integrate.dblquad(moment1_A, 0, Lmax, 0, Lmax)    
    ...
    def moment2_A(L,λ):
        return(kag*((L**3+λ**3)**(2/3)/2-L**2)*sol(L)*sol(λ))
...
def my_ls_func(x,teta):
    ...
    r = integrate.solve_ivp(f2,[x_data[0],x_data[50]],y0,method='BDF',jac=None,t_eval=x_data,rtol=10**-2) 
    #in this case, we only need one of the dependent variable values
    return (r.y[0,:])
...
'''def f_resid(p):
    """ function to pass to optimize.leastsq
        The routine will square and sum the values returned by 
        this function""" 
    sol1=y_data-my_ls_func(x_data,p)
    sol2=y_data1-my_ls_func1(x_data,p)
    return (np.concatenate([sol1,sol2]))'''

guess = [0.001,1,1]
y0 = [m0[0],m1[0], m2[0], m3[0], C[0]]
(params, kvg) = optimize.leastsq(f_resid, guess)
print ( "Growth parameter: kg=", params, "μm/min") 
xeval=np.linspace(min(x_data), max(x_data),51) 

gls = interpolate.UnivariateSpline(xeval, my_ls_func(xeval,params), k=3, s=0)
#gls1=interpolate.UnivariateSpline(xeval, my_ls_func1(xeval,params), k=3, s=0)
#pick a few more points for a very smooth curve, then plot 
#   data and curve fit
xeval=np.linspace(min(x_data), max(x_data),200)
#Plot of the data as red dots and fit as blue line
...
pp.ylabel("m0(#/m3)",{"fontsize":16})
pp.legend(('data','fit'),loc=0)
pp.show()


r1 = integrate.solve_ivp(moments,[x_data[0],x_data[50]],y0,method='BDF',jac=None,t_eval=x_data,rtol=10**-3,args=(params,))   
...
pp.plot(r1.t,r1.y[4,:],'.b',t,C,'-b')
pp.xlabel('t(sec)',{"fontsize":16})
pp.ylabel('C(kg/kg-solv)',{"fontsize":16})
pp.title('Evolution of Concentration')
